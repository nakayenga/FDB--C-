...
WHERE
	 COMPANY_CODE IN (SELECT cmpycode FROM @cmpycodes) and DATEPART(YEAR, RUN_DATE) IN (SELECT yrcode FROM @yrcodes) and DATEPART(QUARTER, RUN_DATE) IN (SELECT qtrcode FROM @qtrcodes)
